...
if __name__ == "__main__":
    ...
    global_pherom = pheromone.Pheromon(size_laby, pos_food, alpha, beta)
    ...
    if rank == 0 :
        ...
        for i in range (1,size) :
                comm.Send(local_pherom.pheromon , dest = i , tag = 3)
    local_ants = Colony(ants_per_proc, pos_nest, max_life)
    ...
    while True:
        ...
        if rank == 0 :
            #print("iteration numéro : " , iter , " Début")
        ...
        if rank != 0  :
            ...
            comm.Recv(local_pherom.pheromon , source=0, tag = 3)
            
            food_counter = local_ants.advance(a_maze, pos_food, pos_nest, local_pherom, food_counter)
            
            comm.send(food_counter , dest = 0 , tag = 1)

            local_pherom.do_evaporation(pos_food)

            comm.Send(local_pherom.pheromon , dest = 0 , tag = 2)

            comm.Send(local_ants.directions , dest = 0 , tag = 4)
            comm.Send(local_ants.historic_path , dest = 0 , tag = 5)
            comm.Send(local_ants.age , dest = 0 , tag = 6)
        ...
        if rank == 0 :
            ...
            for i in range(1,size) :
                local_ants.directions = d.DIR_NONE*np.ones(ants_per_proc, dtype=np.int8)
                local_ants.historic_path = np.zeros((ants_per_proc, max_life+1, 2), dtype=np.int16)
                local_ants.age = np.zeros(ants_per_proc, dtype=np.int64)

                comm.Recv(local_ants.directions  , source=i, tag = 4)
                comm.Recv(local_ants.historic_path , source=i, tag = 5)
                comm.Recv(local_ants.age , source=i, tag = 6)

                local_ants.display(screen)
        ...
        if rank == 0 :
            #print("iteration numéro : " , iter , " Fin")
            #iter+=1
            if food_counter >= 1 and not snapshop_taken:
                pg.image.save(screen, "MyFirstFood.png")
                t_fin = time.time()
                snapshop_taken = True
                print("Temps avant la première nourriture : " , t_fin-t_init , " s")
